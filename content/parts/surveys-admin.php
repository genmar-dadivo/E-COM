    ...
    <div class="container">
        <div class="row">
            <div class="card border-0">
                <div class="card-body">
                    <div class="mt-5">
                        <table id="tsurvey" class="table table-striped responsive nowrap" width="100%">
                            <thead>
                                <tr>
...
<div class="modal fade" id="addsurvey" tabindex="-1" role="dialog">
...
<script>
    function deletesurvey(id) {
        if(confirm("Are you sure to delete this asset? \nSurveys ID: " + id)) {
            $.ajax({
                type: 'POST',
                url: '../../content/action/formdeletesurvey.php',
                data: {eventid: id},
                success: function(data) {
                    alert(data);
                    var tevent = $('#tsurvey').DataTable();
                    tevent.ajax.reload();
                }
            });
        }
    ...
    $('#formSurvey').on('submit', function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '../../content/action/formsurvey.php',
            data: $('#formEvent').serialize(),
            success: function(data) {
                alert(data);
                $('#btnSubmit').prop("disabled", false);
                $("#btnSubmit").html('Submit');
                var thome = $('#tsurvey').DataTable();
                thome.ajax.reload();
                $('#addsurvey').modal('hide');
            }
        });
    });
    $('#tsurvey tfoot th').each( function () {
        var title = $(this).text();
        $(this).html( '<input class="form-control border-1" type="text" placeholder="'+title+'" />' );
    });
    var table = $('#tsurvey').DataTable({
        dom: 'rtip',
        "oLanguage": { "sSearch": "" },
        responsive: true,
        "ajax": {
            'type': 'POST',
            'url': '../../content/action/getSurvey.php?t=1',
        },
